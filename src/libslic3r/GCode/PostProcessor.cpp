...
// host is one of "File", "Repetier", "OctoPrint", "FlashAir" ...
...
// output_name is the final name of the G-code on SD card or when uploaded to OctoPrint.
// If uploading to OctoPrint, then the file will be renamed to output_name first on the target host.
...
bool run_post_process_scripts(std::string &src_path, bool make_copy, const std::string &host, std::string &output_name, const DynamicPrintConfig &config)
{
    ...
    config.setenv_();
    // Let the post-processing script know the target host ("File", "Repetier", "OctoPrint", "FlashAir" ...)
    ...
    // For "OctoPrint", it is a file name optionally with a directory on the target host.
