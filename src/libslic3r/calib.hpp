...
public:
    ...
    CustomGCode::Info generate_custom_gcodes(const DynamicPrintConfig &config, const ModelObject &object, const Vec3d &origin);
...
private:
    void refresh_setup(const DynamicPrintConfig &config, const ModelObject &object, const Vec3d &origin);
    void _refresh_starting_point(const ModelObject &object);
    void _refresh_writer(const ModelObject &object, const Vec3d &origin);
