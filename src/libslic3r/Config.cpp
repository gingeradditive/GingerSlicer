...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or GingerSlicer header.
    ...
    {
        //BBS
        std::string bambuslicer_gcode_header = "; GingerSlicer";

        std::string gingerslicer_gcode_header = std::string("; generated by ");
        gingerslicer_gcode_header += "OrcaSlicer";
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(gingerslicer_gcode_header.c_str(), line_c, strlen(gingerslicer_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
