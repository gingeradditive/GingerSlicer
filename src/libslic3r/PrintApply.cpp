...
Print::ApplyStatus Print::apply(const Model &model, DynamicPrintConfig new_full_config)
{
    ...
    // BBS
    int used_filaments = (int)this->extruders(true).size();

    //new_full_config.normalize_fdm(used_filaments);
    new_full_config.normalize_fdm_1();
    t_config_option_keys changed_keys = new_full_config.normalize_fdm_2((int)objects().size(), used_filaments);
    ...
    int new_used_filaments = (int)this->extruders(true).size();
    t_config_option_keys new_changed_keys = new_full_config.normalize_fdm_2((int)objects().size(), new_used_filaments);
