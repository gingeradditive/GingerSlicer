                            ...
                            bool                        flip)
{
    ...
    CutMesh result;
    result.reserve(static_cast<CutMesh::size_type>(vertices_count), static_cast<CutMesh::size_type>(edges_count), static_cast<CutMesh::size_type>(faces_count));
                            ...
                            const Project     &projection)
{
    ...
    result.reserve(static_cast<CutMesh::size_type>(result.number_of_vertices() + 2 * count_point),
                   static_cast<CutMesh::size_type>(result.number_of_edges() + 4 * count_point),
                   static_cast<CutMesh::size_type>(result.number_of_faces() + 2 * count_point));
    ...
    if (rmap == nullptr) {
        for (FI fi : fis) {
            ...
            for (VI vi : mesh.vertices_around_face(mesh.halfedge(fi))) {
                ...
                if (!vi_cvt.is_valid()) {
                    vi_cvt = VI(static_cast<CutMesh::size_type>(cm.vertices().size()));
