...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in Ginger Slicer before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in Ginger Slicer before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in Ginger Slicer before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in Ginger Slicer before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in Ginger Slicer before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in Ginger Slicer before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in Ginger Slicer before uploading."},
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on Ginger Slicer before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest Ginger Slicer."}
};
...
int CLI::run(int argc, char **argv)
{
    ...
    set_current_thread_name("gingerslicer_main");
    // Save the thread ID of the main thread.
    save_main_thread_id();

    // Ginger: GINGER_THREADS=<n> caps TBB parallelism for the whole process. n=1 makes the
    // slice output reproducible run-to-run (thread scheduling perturbs e.g. the lightning
    // tree growth), which is what the perf-work regression gates diff against. Test/CI knob,
    // no effect when unset.
    static std::optional<tbb::global_control> s_thread_cap;
    if (const char *cap = std::getenv("GINGER_THREADS"); cap != nullptr)
        if (const int n = std::atoi(cap); n > 0)
            s_thread_cap.emplace(tbb::global_control::max_allowed_parallelism, size_t(n));
    ...
    /*#if !defined(_WIN32) && !defined(__APPLE__)
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch GingerSlicer GUI soon";
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current GingerSlicer Version %1%")%GingerSlicer_VERSION;
        ...
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                if (is_bbl_3mf)
                {
                    ...
                    Semver cli_ver = *Semver::parse(GingerSlicer_VERSION);
                    if (!allow_newer_file && ((cli_ver.maj() != file_version.maj()) || (cli_ver.min() < file_version.min()))){
                        BOOST_LOG_TRIVIAL(error) << boost::format("Version Check: File Version %1% not supported by current cli version %2%")%file_version.to_string() %GingerSlicer_VERSION;
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
            ...
            m_print_config.save_to_json(m_config.opt_string(opt_key), std::string("project_settings"), std::string("project"), std::string(GingerSlicer_VERSION));
...
*/
    ...
    if (export_to_3mf) {
        ...
        if (!this->export_project(&m_models[0], export_3mf_file, plate_data_list, project_presets, thumbnails, no_light_thumbnails, top_thumbnails, pick_thumbnails,
                                plate_bboxes, &m_print_config, minimum_save, plate_to_slice - 1))
        {
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into GingerSlicer process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a GingerSlicer process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the GingerSlicer process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"GingerSlicer is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using GingerSlicer.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes GingerSlicer "
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    ...
    boost::nowide::cout
        ...
        << std::endl
        << "Usage: Ginger-Slicer [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
        << std::endl
        << "OPTIONS:" << std::endl;
...
bool CLI::export_project(Model *model, std::string& path, PlateDataPtrs &partplate_data,
    ...
    std::vector<PlateBBoxData*>& plate_bboxes, const DynamicPrintConfig* config, bool minimum_save, int plate_to_export)
{
    bool success = false;
...
extern "C" {
    __declspec(dllexport) int __stdcall gingerslicer_main(int argc, wchar_t **argv)
    {
