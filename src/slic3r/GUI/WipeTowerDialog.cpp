    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize /*,wxPoint(50,50), wxSize(800,350),wxBORDER_RAISED*/)
{
    ...
    m_widget_volume = new SpinInput(this, wxEmptyString, wxString::FromUTF8("mm³"), wxDefaultPosition, wxSize(scale(120), -1),
...
// Parent dialog for purging volume adjustments - it fathers WipingPanel widget (that contains all controls) and a button to toggle
// simple/advanced mode:
WipingDialog::WipingDialog(wxWindow*                       parent,
...
void WipingPanel::create_panels(wxWindow* parent, const int num)
{
    for (size_t i = 0; i < num; i++) {
        ...
        icon->SetBitmap(*get_extruder_color_icon(m_colours[i].GetAsString(wxC2S_HTML_SYNTAX).ToStdString(), std::to_string(i + 1),
...
WipingPanel::WipingPanel(wxWindow*                       parent,
                         const std::vector<float>&       matrix,
                         const std::vector<float>&       extruders,
                         const std::vector<std::string>& extruder_colours,
                         Button*                         calc_button,
    ...
    , m_max_flush_volume(Slic3r::g_max_flush_volume)
{
...
#ifdef _WIN32
            wxTextCtrl* text = new wxTextCtrl(m_page_advanced, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH(), -1),
                                              wxTE_CENTER | wxBORDER_NONE | wxTE_PROCESS_ENTER);
            update_ui(text);
            edit_boxes.back().push_back(text);
#else
            edit_boxes.back().push_back(
                new wxTextCtrl(m_page_advanced, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH(), -1)));
#endif
            if (i == j) {
    ...
    {
        wxString message = _L("Ginger would re-calculate your flushing volumes every time the filaments color changed. You could disable the "
                              "auto-calculate in Ginger Slicer > Preferences");
    ...
    {
        auto multi_desc_label = new wxStaticText(m_page_advanced, wxID_ANY, _(L("Flushing volume (mm³) for each filament pair.")),
        ...
        wxString min_flush_str = wxString::Format(_L("Suggestion: Flushing Volume in range [%d, %d]"),
                                                  *std::min_element(m_min_flush_volume.begin(), m_min_flush_volume.end()),
                                                  m_max_flush_volume);
    ...
    auto add_spin_ctrl = [this](std::vector<wxSpinCtrl*>& vec, float initial) {
        wxSpinCtrl* spin_ctrl = new wxSpinCtrl(m_page_simple, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH(), -1),
