...
void GCodeViewer::_render_calibration_thumbnail_internal(ThumbnailData& thumbnail_data, const ThumbnailsParams& thumbnail_params, PartPlateList& partplate_list, OpenGLManager& opengl_manager)
{
    ...
    auto render_as_batched_model = [](TBuffer& buffer, GLShaderProgram& shader, int position_id, int normal_id) {
        ...
        unsigned int indices_per_instance = static_cast<unsigned int>(buffer.model.data.indices_count());
        ...
        for (size_t j = 0; j < buffer.indices.size(); ++j) {
            ...
            if (position_id != -1) {
                glsafe(::glVertexAttribPointer(position_id, static_cast<GLsizei>(buffer.vertices.position_size_floats()), GL_FLOAT, GL_FALSE, static_cast<GLsizei>(buffer.vertices.vertex_size_bytes()), (const void*)buffer.vertices.position_offset_bytes()));
                glsafe(::glEnableVertexAttribArray(position_id));
            }
            bool has_normals = buffer.vertices.normal_size_floats() > 0;
            if (has_normals) {
                if (normal_id != -1) {
                    glsafe(::glVertexAttribPointer(normal_id, static_cast<GLsizei>(buffer.vertices.normal_size_floats()), GL_FLOAT, GL_FALSE, static_cast<GLsizei>(buffer.vertices.vertex_size_bytes()), (const void*)buffer.vertices.normal_offset_bytes()));
...
void GCodeViewer::update_moves_slider(bool set_to_max)
{
    ...
    std::vector<double> values(static_cast<int>(view.endpoints.last - view.endpoints.first + 1));
    std::vector<double> alternate_values(static_cast<int>(view.endpoints.last - view.endpoints.first + 1));
        ...
        for (const Path& path : t_buffer.paths) {
            ...
            for (size_t j = 1; j < path_vertices_count; ++j) {
                ...
                int interpolation_points_num = gcode_result.moves[move_id].is_arc_move_with_interpolation_points()?
                                                    static_cast<int>(gcode_result.moves[move_id].interpolation_points.size()) : 0;
...
void GCodeViewer::load_shells(const Print& print, bool initialized, bool force_previewing)
{
    ...
    m_shells.print_id = static_cast<unsigned int>(print.id().id);
    ...
    if (m_sequential_view.current.first != m_sequential_view.current.last) {
        for (const auto& [tbuffer_id, ibuffer_id, path_id, sub_path_id] : paths) {
            ...
            if (m_sequential_view.current.first > sub_path.first.s_id) {
                ...
                unsigned int offset = static_cast<unsigned int>(sub_path.first.i_id);
        ...
        for (auto plate : plate_list.get_nonempty_plate_list())
        {
            ...
            for (size_t extruder_id : plate_extruders) {
                ...
                if (plate_print_statistics.model_volumes_per_extruder.find(extruder_id) == plate_print_statistics.model_volumes_per_extruder.end())
                    ...
                    model_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += model_volume;
                }
                if (plate_print_statistics.flush_per_filament.find(extruder_id) == plate_print_statistics.flush_per_filament.end())
                    flushed_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += 0;
                else {
                    double flushed_volume = plate_print_statistics.flush_per_filament.at(extruder_id);
                    flushed_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += flushed_volume;
                }
                if (plate_print_statistics.wipe_tower_volumes_per_extruder.find(extruder_id) == plate_print_statistics.wipe_tower_volumes_per_extruder.end())
                    wipe_tower_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += 0;
                else {
                    double wipe_tower_volume = plate_print_statistics.wipe_tower_volumes_per_extruder.at(extruder_id);
                    wipe_tower_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += wipe_tower_volume;
                }
                if (plate_print_statistics.support_volumes_per_extruder.find(extruder_id) == plate_print_statistics.support_volumes_per_extruder.end())
                    support_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += 0;
                else {
                    double support_volume = plate_print_statistics.support_volumes_per_extruder.at(extruder_id);
                    support_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += support_volume;
                }
            }
...
void GCodeViewer::render_legend(float &legend_height, int canvas_width, int canvas_height, int right_margin)
{
    ...
    auto calculate_offsets = [&imgui, max_width, this](const std::vector<std::pair<std::string, std::vector<::string>>>& title_columns, float extra_size = 0.0f) {
            const ImGuiStyle& style = ImGui::GetStyle();
            std::vector<float> offsets;
    ...
    for (size_t extruder_id : m_extruder_ids) {
        if (m_print_statistics.model_volumes_per_extruder.find(extruder_id) == m_print_statistics.model_volumes_per_extruder.end()) {
            ...
            auto [model_used_filament_m, model_used_filament_g] = get_used_filament_from_volume(volume, static_cast<int>(extruder_id));
    ...
    case EViewType::FeatureType:
    {
        ...
        for(auto item : options_items) {
            if (item != EMoveType::Travel) {
                ...
                append_item(EItemType::Rect, Travel_Colors[0], columns_offsets, true, offsets.back()/*ORCA checkbox_pos*/, visible, [this, item]() {
    ...
    case EViewType::Feedrate:       {
        ...
        append_item(EItemType::None, Travel_Colors[0], { {_u8L("travel"), offsets[0] }}, true, predictable_icon_pos/*ORCA checkbox_pos*/, travel_visible, [this]() {
