    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(48 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#d72828"));
        ...
        try {
            if (type_3mf) {
                ...
                Semver             file_version;
                {
                    ...
                    // BBS: version check
                    Semver app_version = *(Semver::parse(GingerSlicer_VERSION));
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        // do not reset the model config
                        load_config = false;
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by GingerSlicer, load geometry data only."), _L("Load 3mf"));
                    }
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Ginger Slicer, load geometry data only."), _L("Load 3mf"));
                    //         else
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - GingerSlicer");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Ginger Slicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Ginger Slicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    , m_action(2)
{
    ...
    m_fname_f->Wrap(-1);
    m_fname_f->SetForegroundColour(wxColour(48, 38, 38));
    ...
    m_fname_s->Wrap(-1);
    m_fname_s->SetForegroundColour(wxColour(48, 38, 38));
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Ginger Slicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
