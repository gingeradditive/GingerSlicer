...
void KBShortcutsDialog::OnSelectTabel(wxCommandEvent &event)
{
    ...
    while (i != m_hash_selector.end()) {
        Select *sel = i->second;
        if (id == sel->m_index) {
            sel->m_tab_button->SetBackgroundColour(wxColour("#FFC0C0")); // GINGER color for selected tab background
            sel->m_tab_text->SetBackgroundColour(wxColour("#FFC0C0")); // GINGER color for selected tab background
