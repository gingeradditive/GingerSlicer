...
void Selection::paste_objects_from_clipboard()
{
    ...
    for (size_t i=0;i<src_objects.size();i++)
    {
        ...
        if (shift_all(0) != 0 || shift_all(1) != 0) {
            ...
            auto src_offset = src_object->instances.front()->get_offset();
            auto base_offset = src_objects[0]->instances.front()->get_offset();
            auto instance_shift = src_offset - base_offset;
