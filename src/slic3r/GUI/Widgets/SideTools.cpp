...
void SideToolsPanel::doRender(wxDC &dc)
{
    ...
    if (m_none_printer) {
        dc.SetPen(SIDE_TOOLS_BRAND);   // ORCA: Sidebar header background color - Fix for dark mode compability
        dc.SetBrush(SIDE_TOOLS_BRAND); // ORCA: Sidebar header background color - Fix for dark mode compability
        dc.DrawRectangle(0, 0, size.x, size.y);
...
void SideTools::show_status(int status)
{
    if (((status & (int)MonitorStatus::MONITOR_DISCONNECTED) != 0) || ((status & (int)MonitorStatus::MONITOR_DISCONNECTED_SERVER) != 0)) {
        ...
        m_connection_info->SetBackgroundColor(0xd72828);
        m_connection_info->SetBorderColor(0xd72828);
        m_connection_info->Show();
        m_more_button->Hide();
        m_side_error_panel->Hide();
    }
