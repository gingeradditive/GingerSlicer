    ...
    , m_plater(plater), m_export_3mf_cancel(false)
{
    ...
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour( 202, 37, 37), StateColor::Pressed), std::pair<wxColour, int>(wxColour(253, 47, 47), StateColor::Hovered),
    ...
    m_button_refresh->SetTextColor(wxString("#FFFFFE"));
    ...
    m_button_ensure->SetTextColor(wxString("#FFFFFE"));
    ...
    m_statictext_finish->SetForegroundColour(wxColour(215, 40, 40));
...
void SendToPrinterDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
	...
	if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text = _L("The printer is required to be in the same LAN as Ginger Slicer.");
        update_print_status_msg(msg_text, true, true);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
    }
