...
- **Onboarding / editor setup:** see `docs/ginger/DEV_SETUP.md`. clangd is OPTIONAL (VS uses native IntelliSense); the committed `.clangd` points at `build-cdb`, and `docs/ginger/vscode-settings.sample.json` has the `--query-driver=**/cl.exe` flag clangd needs for MSVC headers.
...
- **`pwsh` is NOT installed — use `powershell`.** This machine only has Windows PowerShell 5.1. Invoke `.ps1` scripts with `powershell -ExecutionPolicy Bypass -File scripts/<name>.ps1` (NOT `pwsh ...`, which fails with `CommandNotFoundException`). The `pwsh` usages shown in the "Profiles distribution & CI" examples are illustrative — substitute `powershell` when actually running.
