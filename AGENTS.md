...
- **C++ symbol search:** if `compile_commands.json` exists (run `scripts/gen_compile_commands.ps1`), prefer LSP-driven tools over regex. Without it, fall back to `grep_search` on declarations.
